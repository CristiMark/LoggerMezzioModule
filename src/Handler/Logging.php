...
class Logging
{
    ...
    public function __construct(
        ...
        $this->logPath                   = $errorHeroModuleLocalConfig['logpath'];
    ...
    private function collectErrorExceptionData(Throwable $t): array
    {
        ...
        if ($t instanceof Exception\InvalidQueryException) {
            $errorQuery = substr($t->getMessage(), strpos($t->getMessage(), "sql: ") + 5);
        } else {
            $errorQuery = null;
        }
    ...
    private function collectErrorExceptionExtraData(array $collectedExceptionData, RequestInterface $request): array
    {
        ...
        return [
            ...
            'url' => $url,
            'query' => $collectedExceptionData['errorQuery'],
    ...
    Private function SendInFile( string $path,int $priority, string $errorMessage, array $extra){


        $writer = new Writer\Stream($path);
            (new Logger())->addWriter($writer)
                ->log($priority, $errorMessage, $extra);
    ...
    public function handleErrorException(Throwable $t, RequestInterface $request): void
    {
        ...
        try {
            ...
            if ($collectedExceptionData['errorMessage'] == "Connection error") {
                $this->SendInFile($this->logPath,
                    $collectedExceptionData['priority'],
                    $collectedExceptionData['errorMessage'], $extra);
            }

            if (
            $this->isExists(
                $collectedExceptionData['trace'],
